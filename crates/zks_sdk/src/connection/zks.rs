...
use zks_wire::{signaling::SignalingClientTrait, SwarmCircuit, SwarmController};
...
impl<S: SignalingClientTrait + 'static> ZksConnection<S> {
    ...
    where
        S: From<zks_wire::signaling::SignalingClient>,
    {
        info!(
            "🔐 Connecting to ZKS peer via onion routing: {} with {}-{} hops",
            url, min_hops, max_hops
        );
        ...
        let target_peer = parsed_url
            .host_str()
            .ok_or_else(|| SdkError::InvalidUrl("Missing host in URL".to_string()))?
            .to_string();

        // Create swarm controller with platform detection
        let swarm_controller = SwarmController::<S>::new().await.map_err(|e| {
            SdkError::ConnectionFailed(format!("Failed to create swarm controller: {}", e))
        })?;
        ...
        swarm_controller
            .connect(signaling_url, local_peer_id)
            .await
            .map_err(|e| {
                SdkError::ConnectionFailed(format!("Failed to connect to signaling server: {}", e))
            })?;
        ...
        info!(
            "Building onion circuit to {} with {}-{} hops",
            target_peer, min_hops, max_hops
        );
        let circuit_id = swarm_controller
            .build_onion_circuit(&target_peer, min_hops, max_hops)
            .await
            .map_err(|e| {
                SdkError::ConnectionFailed(format!("Failed to build onion circuit: {}", e))
            })?;

        info!(
            "🧅 Onion circuit {} established with {} hops",
            circuit_id, max_hops
        );

        // Create an onion stream that routes through the circuit
        let onion_stream = swarm_controller
            .create_onion_stream(&circuit_id)
            .await
            .map_err(|e| {
                SdkError::ConnectionFailed(format!("Failed to create onion stream: {}", e))
            })?;
        ...
        info!(
            "🔐 ZKS connection established with {} via onion circuit {} ({} hops)",
            peer_addr, circuit_id, max_hops
        );
    ...
    pub async fn recv(&mut self, buf: &mut [u8]) -> Result<usize> {
        debug!(
            "Receiving data from {} ({} hops)",
            self.peer_addr, self.hop_count
        );
    ...
    pub async fn build_circuit(&mut self, min_hops: u8, max_hops: u8) -> Result<()> {
        if let Some(swarm_controller) = &self.swarm_controller {
            info!(
                "Building circuit with {}-{} hops via swarm controller",
                min_hops, max_hops
            );
            ...
            let circuit_id = swarm_controller
                ...
                .await
                .map_err(|e| {
                    SdkError::ConnectionFailed(format!("Failed to build circuit: {}", e))
                })?;
            ...
            Err(SdkError::ConnectionFailed(
                "No swarm controller available for circuit building".to_string(),
            ))
        }
    }
    ...
    pub async fn send_onion(&mut self, data: &[u8]) -> Result<()> {
        if let Some(circuit) = &self.circuit {
            debug!(
                "Onion routing {} bytes through {} hops",
                data.len(),
                circuit.hop_count()
            );
    ...
    pub async fn recv_onion(&mut self, buf: &mut [u8]) -> Result<usize> {
        ...
        if has_circuit {
            ...
            if let Some(circuit) = &self.circuit {
                ...
                let decrypted = circuit.onion_decrypt(&encrypted_buf[..n]).map_err(|e| {
                    SdkError::NetworkError(format!("Onion decryption failed: {}", e))
                })?;
    ...
    pub async fn close(mut self) -> Result<()> {
        info!(
            "Closing ZKS connection to {} ({} hops)",
            self.peer_addr, self.hop_count
        );
