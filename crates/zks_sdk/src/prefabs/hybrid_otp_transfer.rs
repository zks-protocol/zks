...
impl HybridOtpFileTransfer {
    ...
    pub async fn encrypt_file(&self, file_path: &Path, entropy: &[u8; 32]) -> Result<PathBuf> {
        info!(
            "Encrypting file: {:?} with Hybrid computational encryption",
            file_path
        );
        ...
        let encrypted_data =
            zks_crypt::hybrid_computational::HybridOtp::new(self.entropy_source.clone())
                .encrypt_with_sync(&content, entropy)
                .await?;
        ...
        tokio::fs::write(&output_path, &encrypted_data)
            .await
            .map_err(SdkError::IoError)?;
    ...
    pub async fn decrypt_file(&self, encrypted_path: &Path, entropy: &[u8; 32]) -> Result<PathBuf> {
        info!(
            "Decrypting file: {:?} with Hybrid computational encryption",
            encrypted_path
        );
...
mod tests {
    ...
    async fn test_encrypt_decrypt_file() {
        ...
        tokio::fs::write(&file_path, b"Hello, Hybrid OTP!")
            .await
            .unwrap();

        // Encrypt
        let encrypted_path = transfer
            .encrypt_file(&file_path, &sync_entropy)
            .await
            .unwrap();
        assert!(encrypted_path.ends_with(".hotp"));

        // Decrypt
        let decrypted_path = transfer
            .decrypt_file(&encrypted_path, &sync_entropy)
            .await
            .unwrap();
