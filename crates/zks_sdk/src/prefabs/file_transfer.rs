...
impl SecureFileTransfer {
        ...
        F: FnMut(u64, u64),
    {
        ...
        let metadata = std::fs::metadata(path).map_err(|e| SdkError::IoError(e))?;
        ...
        let mut file = File::open(path).await.map_err(SdkError::IoError)?;

        let mut buffer = vec![0u8; self.chunk_size];
        let mut sent = 0u64;

        loop {
            let n = file.read(&mut buffer).await.map_err(SdkError::IoError)?;
        ...
        F: FnMut(u64, u64),
    {
        ...
        let metadata_str =
            String::from_utf8(metadata).map_err(|e| SdkError::SerializationError(e.to_string()))?;

        let parts: Vec<&str> = metadata_str.split(':').collect();
        if parts.len() != 2 {
            return Err(SdkError::InvalidUrl(
                "Invalid file metadata format".to_string(),
            ));
        }
        ...
        let mut file = File::create(&save_path).await.map_err(SdkError::IoError)?;
        ...
        while received < file_size {
            ...
            file.write_all(&chunk).await.map_err(SdkError::IoError)?;
        ...
        S: SignalingClientTrait,
    {
        ...
        let metadata = file.metadata().await.map_err(SdkError::IoError)?;
        ...
        loop {
            let n = reader.read(&mut buffer).await.map_err(SdkError::IoError)?;
        ...
        F: FnMut(u64, u64),
    {
        ...
        // In a full implementation, this would handle swarm-specific optimizations
        self.recv_file_zks_impl(connection, save_path, on_progress)
            .await
    }
        ...
        F: FnMut(u64, u64),
    {
        ...
        file.flush().await.map_err(SdkError::IoError)?;
