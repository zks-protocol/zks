...
pub fn derive_session_key(shared_secret: &[u8], entropy: &[u8], info: &str) -> Result<[u8; 32]> {
    use hkdf::Hkdf;
    use sha2::Sha256;
...
pub fn generate_random_bytes(len: usize) -> Result<Vec<u8>> {
    ...
    debug!(
        "Generated {} high-entropy random bytes (drand XOR OsRng)",
        len
    );
    Ok(bytes.to_vec())
}
...
pub async fn ml_kem_key_exchange() -> Result<(Vec<u8>, Vec<u8>)> {
    ...
    debug!(
        "Generated ML-KEM keypair (public: {} bytes, secret: {} bytes)",
        public_key.len(),
        secret_key.len()
    );
...
pub async fn ml_dsa_sign(message: &[u8], secret_key: &[u8]) -> Result<Vec<u8>> {
    ...
    if secret_key.len() != ML_DSA_SECRET_KEY_SIZE {
        return Err(SdkError::CryptoError(
            format!(
                "Invalid ML-DSA-87 secret key size: expected {} bytes, got {}",
...
pub async fn ml_dsa_keypair() -> Result<(Vec<u8>, Vec<u8>)> {
    ...
    debug!(
        "Generated ML-DSA keypair (vk: {} bytes, sk: {} bytes)",
        public_key.len(),
        secret_key.len()
    );
