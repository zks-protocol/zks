...
use zks_wire::signaling::{PeerCapabilities, PeerInfo};
...
async fn create_real_signaling_client(room_id: &str) -> Arc<CloudflareSignalingClient> {
    ...
    let client = CloudflareSignalingClient::connect(config, room_id.to_string())
        .await
...
async fn test_end_to_end_circuit_creation() {
    ...
    info!(
        "✅ Successfully created circuit {} with real ML-KEM handshakes",
        circuit_id
    );

    // Verify circuit structure using public API
    let circuit_info = manager
...
async fn test_circuit_data_transmission() {
    ...
    let received_data: Vec<u8> = manager
        .receive_from_swarm_network(circuit_id)
        .await
...
async fn test_multiple_circuits_isolation() {
    ...
    let circuit1_id = manager
        .create_circuit(&room_id, 2)
        .await
        .expect("Failed to create circuit 1");
    let circuit2_id = manager
        .create_circuit(&room_id, 3)
        .await
        .expect("Failed to create circuit 2");

    // Verify both circuits exist and are independent
    let circuit1_info = manager
        .get_circuit_info(circuit1_id)
        .await
        .expect("Failed to get circuit 1 info");
    let circuit2_info = manager
        .get_circuit_info(circuit2_id)
        .await
    ...
    assert!(
        all_circuits.contains(&circuit1_id),
        "Should contain circuit 1"
    );
    assert!(
        all_circuits.contains(&circuit2_id),
        "Should contain circuit 2"
    );
...
async fn test_phase_3_complete_integration() {
    ...
    // Send data through the circuit
    manager
        .send_via_swarm(circuit_id, test_message)
        .await
    ...
    assert!(
        ...
        "Should receive data from complete circuit"
    );
