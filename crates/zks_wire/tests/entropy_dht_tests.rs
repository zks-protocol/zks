...
use zks_wire::{
    dht_lookup::{DHTLookupConfig, DHTLookupService},
    entropy_cache::{EntropyCache, EntropyCacheConfig},
    entropy_swarm::{EntropyBlockProvider, EntropySwarm, EntropySwarmConfig},
    swarm::Swarm,
    PeerId,
};
...
fn create_test_entropy_block(start_round: u64, end_round: u64) -> EntropyBlock {
    ...
    for round_num in start_round..=end_round {
        ...
        block
            .add_round(round)
            .expect(&format!("Failed to add round {}", round_num));
    }
...
async fn test_entropy_cache_basic_operations() {
    ...
    cache
        ...
        .await
        .expect("Failed to store block1");
    cache
        .store_block(block2.clone())
        .await
        .expect("Failed to store block2");
    ...
    let non_existent = cache
        .get_block(999)
        .await
        .expect("Failed to get non-existent block");
...
async fn test_entropy_cache_lru_eviction() {
    ...
    cache
        ...
        .await
        .expect("Failed to store block3");

    // This should evict the least recently used block (block1)
    cache
        .store_block(block4.clone())
        .await
        .expect("Failed to store block4");
...
async fn test_provider_record_management() {
    ...
    cache
        .add_provider_record(1, peer1.to_string())
        .await
        .expect("Failed to add provider1");
    cache
        .add_provider_record(1, peer2.to_string())
        .await
        .expect("Failed to add provider2");
    cache
        .add_provider_record(2, peer3.to_string())
        .await
        .expect("Failed to add provider3");
    ...
    let removed = cache
        .remove_provider_record(1, &peer1.to_string())
        .await
...
async fn test_entropy_swarm_block_validation() {
    ...
    let result = swarm
        .handle_received_block(valid_block, "test_peer".to_string())
        .await;
    assert!(result.is_ok());
    ...
    let result = swarm
        .handle_received_block(corrupted_block, "test_peer".to_string())
        .await;
...
async fn test_cache_cleanup_expired() {
    ...
    let retrieved_after = cache
        .get_block(1)
        .await
        .expect("Failed to get block after cleanup");
...
async fn test_concurrent_cache_operations() {
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            ...
            cache_clone
                .add_provider_record(start_round, peer_id.to_string())
                .await
