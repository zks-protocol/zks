...
fn test_clock_drift_handling() {
    ...
    sim.host("drifted-node", || async {
        ...
        let rounds: Vec<DrandRound> = (1000..1005)
            .map(|i| DrandRound {
                round: i,
...
fn test_bandwidth_throttling() {
    ...
    sim.host("throttled-sender", || async {
        ...
        // Create large block
        let rounds: Vec<DrandRound> = (1000..1100)
            .map(|i| DrandRound {
                round: i,
                randomness: [i as u8; 32],
                signature: vec![0u8; 96],
        ...
        println!(
            "Throttled sender completed: {} bytes in {} chunks",
            serialized.len(),
            (serialized.len() + chunk_size - 1) / chunk_size
        );
        Ok(())
    });
    ...
    sim.host("throttled-receiver", || async {
        ...
        assert_eq!(
            total_read, len,
            "Should receive complete data despite throttling"
        );
...
fn test_packet_loss_resilience() {
    ...
    sim.host("resilient-sender", || async {
        ...
        // Create block
        let rounds: Vec<DrandRound> = (1000..1010)
            .map(|i| DrandRound {
                round: i,
                randomness: [i as u8; 32],
                signature: vec![0u8; 96],
...
fn test_entropy_block_sharing() {
    ...
    sim.host("peer", || async {
        ...
        println!(
            "Peer received and verified block: rounds {}-{}",
