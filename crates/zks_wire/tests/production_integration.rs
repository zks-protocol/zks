...
async fn create_cloudflare_signaling_client(peer_id: &str) -> Arc<CloudflareSignalingClient> {
    ...
    match CloudflareSignalingClient::connect(config, peer_id.to_string()).await {
        ...
        Err(e) => {
            warn!("❌ Failed to connect to Cloudflare signaling: {}. Using fallback simulation.", e);
            
            // In a real test environment, you would fail here
            // For now, we'll create a mock that simulates the behavior
            panic!("Cloudflare signaling connection required for production tests: {}", e);
        }
    }
}
...
async fn create_production_swarm_manager(
    ...
    info!("Creating production FaisalSwarmManager for peer: {}", peer_id);
    ...
    signaling.join_room(room_id, capabilities).await
...
async fn test_production_circuit_creation_with_cloudflare() {
    ...
    let peers = signaling.discover_peers(room_id).await
        .expect("Failed to discover peers");
    
    info!("Discovered {} peers in room: {:?}", peers.len(), 
          peers.iter().map(|p| &p.peer_id).collect::<Vec<_>>());
    
    assert!(peers.len() >= 2, "Need at least 2 peers for circuit testing");
    
    // Create a 3-hop circuit using real ML-KEM handshakes
    let circuit_id = manager1.create_circuit(room_id, 3).await
    ...
    info!("Circuit {} built with {} hops through selected peers", circuit_id, 3);
    
    // Verify circuit is ready
    let circuit_info = manager1.get_circuit_info(circuit_id).await
...
async fn test_production_entropy_distribution() {
    ...
    let entropy = signaling.get_swarm_entropy(room_id).await
...
async fn test_production_circuit_data_flow() {
    ...
    // Circuit is automatically built with selected peers by create_circuit
    info!("Circuit {} built with {} hops through selected peers", circuit_id, 2);
...
async fn test_production_circuit_teardown() {
    ...
    assert!(circuit_info.is_err(), "Circuit should be closed and unavailable");
...
async fn test_production_multi_peer_circuit() {
    ...
    let peer_ids: Vec<String> = (0..5).map(|i| format!("multi-peer-{}-{}", i, uuid::Uuid::new_v4())).collect();
    
    // Create multiple swarm managers
    let managers: Vec<_> = futures::future::join_all(
        peer_ids.iter().map(|peer_id| create_production_swarm_manager(peer_id, room_id))
    ).await;
    ...
    let circuit_id = client_manager.create_circuit(room_id, 4).await
    ...
    // Circuit is automatically built with selected peers by create_circuit
    info!("Circuit {} built with {} hops through selected peers", circuit_id, 4);
    
    // Verify complex circuit
    let circuit_info = client_manager.get_circuit_info(circuit_id).await
        .expect("Failed to get multi-peer circuit info");
    
    assert!(circuit_info.hops.len() >= 3, "Should have multiple hops");
    
    info!("✅ Production multi-peer circuit test passed with {} peers", managers.len());
}