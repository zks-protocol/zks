...
impl StunClient {
    ...
    async fn resolve_stun_server(server_addr: &str) -> Result<SocketAddr> {
        ...
        if parts.len() != 2 {
            return Err(WireError::stun(
                "Invalid STUN server format, expected domain:port",
            ));
        }
        ...
        let port: u16 = parts[1]
            .parse()
            .map_err(|_| WireError::stun("Invalid port number"))?;
            ...
            Err(_) => {
                ...
                let ip = match domain {
                    "stun.l.google.com" => "74.125.250.129"
                        .parse()
                        .map_err(|_| WireError::stun("Failed to parse IP"))?,
                    "stun1.l.google.com" => "74.125.250.130"
                        .parse()
                        .map_err(|_| WireError::stun("Failed to parse IP"))?,
                    "stun2.l.google.com" => "74.125.250.131"
                        .parse()
                        .map_err(|_| WireError::stun("Failed to parse IP"))?,
                    "stun3.l.google.com" => "74.125.250.132"
                        .parse()
                        .map_err(|_| WireError::stun("Failed to parse IP"))?,
                    "stun4.l.google.com" => "74.125.250.133"
                        .parse()
                        .map_err(|_| WireError::stun("Failed to parse IP"))?,
                    "stun.services.mozilla.com" => "52.86.224.165"
                        .parse()
                        .map_err(|_| WireError::stun("Failed to parse IP"))?,
                    "stun.stunprotocol.org" => "34.226.115.224"
                        .parse()
                        .map_err(|_| WireError::stun("Failed to parse IP"))?,
                    _ => {
                        return Err(WireError::stun(&format!(
                            "DNS resolution failed for unknown STUN server: {}",
                            domain
                        )))
                    }
                };
    ...
    pub async fn discover(&mut self) -> Result<SocketAddr> {
        ...
        for server in &self.servers {
            ...
            match self.query_stun_server(socket, *server).await {
                Ok(public_addr) => {
                    info!(
                        "Discovered public address: {} via STUN server: {}",
                        public_addr, server
                    );
    ...
    fn parse_binding_response(&self, response: &[u8]) -> Result<SocketAddr> {
        ...
        if msg_type != 0x0101 {
            ...
            return Err(WireError::InvalidMessage(
                "Not a binding response".to_string(),
            ));
        }
        ...
        Err(WireError::InvalidMessage(
            "No XOR-MAPPED-ADDRESS found".to_string(),
        ))
    }
    ...
    fn parse_xor_mapped_address(&self, data: &[u8]) -> Result<SocketAddr> {
        ...
        match family {
            0x01 => {
                // IPv4
                let xor_addr =
                    u32::from_be_bytes([data[4], data[5], data[6], data[7]]) ^ 0x2112A442;
            ...
            0x02 => {
                ...
                Err(WireError::InvalidMessage(
                    "IPv6 support not implemented".to_string(),
                ))
            }
