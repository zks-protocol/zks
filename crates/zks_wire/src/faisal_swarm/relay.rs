...
impl RelayHandler {
    ...
    pub async fn handle_request(
        ...
        info!("🔄 Relay {} processing request for circuit {}", self.local_peer_id, request.circuit_id);
        ...
        match circuit_state {
            Some(state) => {
                ...
                self.handle_existing_circuit(request, state, from_peer).await
            }
            None => {
    ...
    async fn handle_existing_circuit(
        ...
        debug!("Processing existing circuit {} at position {}", state.circuit_id, state.position);
        ...
        match command {
            CellCommand::Forward { data } => {
                // Forward to next hop
                self.forward_to_next_hop(state.circuit_id, data, &state).await
            }
            CellCommand::Process { data } => {
                // We're the exit node - process the data
                self.process_exit_data(state.circuit_id, data).await
            }
            CellCommand::Extend { next_hop, next_hop_addr, client_pk } => {
                // Extend the circuit to the next hop
                self.handle_extend(state.circuit_id, &state, next_hop, next_hop_addr, client_pk).await
            }
    ...
    async fn handle_new_circuit(
        ...
        info!("🛡️  Processing new circuit {} as guard node", request.circuit_id);
        ...
        let shared_secret: [u8; 32] = encapsulation.shared_secret.as_slice().try_into()
        ...
        let mut response_cipher = WasifVernam::new(shared_secret)
             .map_err(|e| SwarmError::Encryption(format!("Failed to create response cipher: {:?}", e)))?;
        ...
        self.establish_shared_secret(circuit_id, shared_secret, None, Some(from_peer), 0, response_cipher).await?;
    ...
    fn parse_cell_command(&self, data: Vec<u8>) -> Result<CellCommand> {
        ...
        match data[0] {
            0x01 => Ok(CellCommand::Forward { data: data[1..].to_vec() }),
            0x02 => Ok(CellCommand::Process { data: data[1..].to_vec() }),
            0x03 => Ok(CellCommand::Teardown),
            0x04 => {
                // EXTEND: [cmd:1][peer_id_len:2][peer_id_bytes][addr_len:2][addr_bytes][pk_len:2][pk_bytes]
                if data.len() < 7 { // Minimum size check
                ...
                std::io::Read::read_exact(&mut cursor, &mut peer_id_bytes)
                    .map_err(|_| SwarmError::Serialization("Failed to read peer_id bytes".into()))?;
                ...
                let next_hop_addr: Multiaddr = s.parse()
                ...
                Ok(CellCommand::Extend { next_hop, next_hop_addr, client_pk })
            }
    ...
    async fn forward_to_next_hop(
        ...
        if let Some(next_hop) = state.next_hop {
            ...
            // Get transport
            let transport_arc = self.p2p_transport.as_ref()
                .ok_or_else(|| SwarmError::InternalError("Transport not initialized in relay".into()))?
                .upgrade()
            ...
            let response = transport.send_faisal_request(next_hop, request).await
            ...
            let encrypted_response_data = cipher.encrypt_sequenced(&response.data)
                .map_err(|e| SwarmError::Encryption(format!("Response encryption failed: {:?}", e)))?;
            ...
            Err(SwarmError::Protocol(format!("No next hop for circuit {}", circuit_id)))
        }
    }
    ...
    async fn handle_extend(
        ...
        info!("🔗 Extending circuit {} to next hop {}", circuit_id, next_hop);
        ...
        // But Dial is async.
        {
            let mut transport: tokio::sync::RwLockWriteGuard<NativeP2PTransport> = transport_arc.write().await;
        ...
        let response = {
             let mut transport = transport_arc.write().await;
             transport.send_faisal_request(next_hop, create_request).await
                .map_err(|e| SwarmError::Network(format!("Extend request to next hop failed: {}", e)))?
        };
        
        if !response.success {
            return Err(SwarmError::HandshakeFailed(format!("Next hop {} rejected extend request", next_hop)));
        }
        ...
        // Also update connections map? Ideally yes.
        {
            ...
            conns.insert(next_hop, RelayConnection {
                peer_id: next_hop,
        ...
        let encrypted_extended_payload = cipher.encrypt_sequenced(&response.data)
            .map_err(|e| SwarmError::Encryption(format!("Extend response encryption failed: {:?}", e)))?;
    ...
    async fn process_exit_data(&self, circuit_id: u32, data: Vec<u8>) -> Result<FaisalSwarmResponse> {
        ...
        let encrypted_data = cipher.encrypt_sequenced(&data)
            .map_err(|e| SwarmError::Encryption(format!("Exit response encryption failed: {:?}", e)))?;
    ...
    async fn handle_circuit_teardown(
        ...
        let encrypted_conf = cipher.encrypt_sequenced(confirmation)
            .map_err(|e| SwarmError::Encryption(format!("Teardown ACK encryption failed: {:?}", e)))?;
    ...
    pub async fn establish_shared_secret(
        ...
        info!("🔑 Established shared secret for circuit {} at position {}", circuit_id, position);
...
mod tests {
    ...
    async fn test_extend_parsing() {
        ...
        match result {
            CellCommand::Extend { next_hop: nh, next_hop_addr: nha, client_pk: pk } => {
