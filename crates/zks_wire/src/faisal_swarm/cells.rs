...
impl CellHeader {
    ...
    pub fn from_bytes(bytes: &[u8; CELL_HEADER_SIZE]) -> Result<Self, CellError> {
        ...
        let command =
            CellCommand::from_u8(command_byte).ok_or(CellError::InvalidCommand(command_byte))?;
...
impl FaisalSwarmCell {
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        debug!(
            "Serialized cell: {} bytes ({} padding)",
            CELL_SIZE, padding_len
        );
...
impl<S: crate::signaling::SignalingClientTrait> CellProcessor<S> {
    ...
    pub async fn process_cell(
    ...
    ) -> Result<Vec<FaisalSwarmCell>, CellError> {
        ...
        debug!(
            "Processing cell: {:?} for circuit {}",
            cell.header.command, cell.header.circuit_id
        );
    ...
    /// Process relay cell
    async fn process_relay_cell(
        &self,
        cell: FaisalSwarmCell,
    ) -> Result<Vec<FaisalSwarmCell>, CellError> {
        debug!(
            "Processing relay cell for circuit {}",
            cell.header.circuit_id
        );

        // Deserialize the relay payload
        let relay_payload =
            RelayPayload::from_bytes(&cell.payload).map_err(|_| CellError::InvalidRelayPayload)?;

        debug!(
            "Relay command: {:?}, Stream ID: {}",
            relay_payload.relay_command, relay_payload.stream_id
        );
        ...
        match relay_payload.relay_command {
            ...
            RelayCommand::Begin => {
                ...
                debug!(
                    "Creating new stream {} on circuit {}",
                    relay_payload.stream_id, cell.header.circuit_id
                );
            ...
            RelayCommand::Connected => {
                ...
                debug!(
                    "Stream {} already connected on circuit {}",
                    relay_payload.stream_id, cell.header.circuit_id
                );
                Ok(vec![])
            }
            RelayCommand::Resolve | RelayCommand::Resolved => {
                ...
                debug!(
                    "DNS resolution not implemented for circuit {}",
                    cell.header.circuit_id
                );
    ...
    async fn process_vernam_relay_cell(
        &self,
        cell: FaisalSwarmCell,
    ) -> Result<Vec<FaisalSwarmCell>, CellError> {
        debug!(
            "Processing Vernam relay cell for circuit {}",
            cell.header.circuit_id
        );
...
pub fn create_relay_cell(
    circuit_id: u32,
    stream_id: u16,
    data: Vec<u8>,
) -> Result<FaisalSwarmCell, CellError> {
