...
use crate::swarm::{ProviderRecord, Swarm};
...
impl DHTLookupManager {
    ...
    pub async fn lookup_entropy_block(
        &self,
        start_round: u64,
        ...
        if self.active_lookups.read().await.contains_key(&start_round) {
            debug!(
                "Lookup for entropy block {} already in progress",
                start_round
            );
    ...
    async fn perform_lookup_with_retry(
        ...
        loop {
            ...
            if attempt > 0 {
                info!(
                    "Retrying DHT lookup for entropy block {} (attempt {})",
                    start_round,
                    attempt + 1
                );
            ...
            match self.perform_single_lookup(start_round).await {
                ...
                Err(e) => {
                    warn!(
                        "DHT lookup attempt {} failed for entropy block {}: {}",
                        attempt + 1,
                        start_round,
                        e
                    );
    ...
    async fn perform_single_lookup(
        &self,
        start_round: u64,
    ) -> Result<DHTLookupResult, DHTLookupError> {
        ...
        info!(
            "Found {} providers for entropy block {}",
            providers.len(),
            start_round
        );
    ...
    pub fn select_best_provider(providers: &[ProviderRecord]) -> Option<ProviderRecord> {
        ...
        providers.iter().find(|p| !p.addresses.is_empty()).cloned()
    }
