...
impl EntropySeeder {
    ...
    pub async fn new(config: SeederConfig) -> SeederResult<Self> {
        ...
        let cache = if config.cache_blocks {
            Some(Arc::new(EntropyCache::new(
                crate::entropy_cache::EntropyCacheConfig::default(),
            )))
        } else {
            None
        };
    ...
    pub async fn with_drand_client(
        config: SeederConfig,
        drand_client: Arc<DrandEntropy>,
    ) -> SeederResult<Self> {
        let cache = if config.cache_blocks {
    ...
    pub async fn start(&mut self) -> SeederResult<()> {
        ...
        if *self.running.read().await {
            return Err(SeederError::AlreadyRunning(
                "Seeder is already running".to_string(),
            ));
        }
        ...
        let current_round = match self.get_current_round().await {
            ...
            Err(e) => {
                ...
                match self.estimate_current_round() {
                    ...
                    Err(e) => {
                        return Err(SeederError::InitializationError(format!(
                            "Failed to determine starting round: {}",
                            e
                        )));
                    }
                }
        ...
        let start_round = if self.config.start_from_current {
            ...
            if configured_start < current_round {
                info!(
                    "Configured start round {} is behind current round {}, catching up to current",
                    configured_start, current_round
                );
                current_round
            } else {
                configured_start
    ...
    pub async fn get_runtime_stats(&self) -> RuntimeStats {
        ...
        RuntimeStats {
            ...
            success_rate: if stats.successful_operations + stats.failed_operations > 0 {
                stats.successful_operations as f64
    ...
    async fn run_polling_loop(&self, mut current_round: u64) {
        ...
        while *self.running.read().await {
            ...
            if consecutive_errors >= MAX_CONSECUTIVE_ERRORS {
                error!(
                    "Too many consecutive errors ({}), stopping seeder",
                    consecutive_errors
                );
                *self.running.write().await = false;
                break;
            ...
            match self.get_current_round().await {
                Ok(latest_round) => {
                    ...
                    if current_round < latest_round {
                        ...
                        info!(
                            "Seeder is behind (current: {}, latest: {}), catching up...",
    ...
    async fn process_round_range(&self, start_round: u64) -> SeederResult<u64> {
        ...
        if self.config.block_size == 0 {
            return Err(SeederError::ConfigError(
        ...
        let end_round = start_round
            ...
            .ok_or_else(|| {
                SeederError::ConfigError(format!(
        ...
        let rounds = self
            .drand_client
            .fetch_range(start_round, end_round)
            .await
        ...
        if rounds.is_empty() {
            debug!(
                "No rounds available for range {}-{} (seeder ahead of beacon)",
        ...
        if let Some(ref cache) = self.cache {
            cache.store_block(block.clone()).await.map_err(|e| {
                SeederError::BlockCreationError(format!("Failed to cache block: {}", e))
            })?;
        }
        ...
        if self.config.auto_publish {
            if let Some(ref swarm) = self.swarm {
                swarm
                    .publish_entropy_block(block.clone())
                    .await
                    .map_err(|e| {
                        SeederError::PublishError(format!("Failed to publish block: {}", e))
                    })?;
    ...
    async fn create_entropy_block(&self, rounds: Vec<DrandRound>) -> SeederResult<EntropyBlock> {
        if rounds.is_empty() {
            return Err(SeederError::BlockCreationError(
                "No rounds provided".to_string(),
            ));
        }
        ...
        let block = EntropyBlock::from_rounds(rounds).map_err(|e| {
            SeederError::BlockCreationError(format!("Failed to create block: {}", e))
        })?;
        ...
        info!(
            "Created EntropyBlock {} with rounds {}-{}",
