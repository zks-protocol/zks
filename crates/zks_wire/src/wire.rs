...
impl WireMessage {
    ...
    pub fn peer_discovery(sequence: u32, peer_info: &[u8]) -> Self {
        Self::new(
            ...
            sequence,
            Bytes::copy_from_slice(peer_info),
        )
    }
    ...
    pub fn nat_traversal(sequence: u32, traversal_data: &[u8]) -> Self {
        Self::new(
            MessageType::NatTraversal,
            sequence,
            Bytes::copy_from_slice(traversal_data),
        )
    }
...
mod tests {
    ...
    fn test_message_type_conversion() {
        ...
        assert_eq!(
            MessageType::from_u8(0x03).unwrap(),
            MessageType::HandshakeFinish
        );
        assert_eq!(
            MessageType::from_u8(0x04).unwrap(),
            MessageType::EncryptedData
        );
    ...
    fn test_wire_protocol_encoding() {
        ...
        let encoded = protocol
            .encode(MessageType::Keepalive, payload.clone())
            .unwrap();
