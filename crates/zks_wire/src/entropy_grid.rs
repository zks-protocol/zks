...
fn verify_entropy_quality(randomness: &[u8; 32], min_threshold: f64) -> Result<f64, DrandError> {
    ...
    if randomness.iter().all(|&b| b == 0) {
        return Err(DrandError::ParseError(
            "Entropy is all zeros - possible attack".to_string(),
        ));
    }

    if randomness.iter().all(|&b| b == 0xFF) {
        return Err(DrandError::ParseError(
            "Entropy is all 0xFF - possible attack".to_string(),
        ));
    }
...
impl EntropyGrid {
    ...
    pub fn new(config: EntropyGridConfig, drand_client: Arc<DrandEntropy>) -> Self {
        Self {
            config,
            drand_client,
    ...
    pub async fn get_round(&self, round_number: u64) -> Result<DrandRound, DrandError> {
        ...
        if self.config.enable_cache {
            ...
            if let Some(ref cache) = *cache_guard {
                ...
                match timeout(
                    fetch_timeout,
                    self.get_round_from_cache(cache_clone.as_ref(), round_number),
                )
                .await
                {
                    Ok(Ok(round)) => {
                    ...
                    Err(_) => {
                        warn!(
                            "Cache fetch for round {} timed out after {}s",
                            round_number, self.config.fetch_timeout_secs
                        );
                    }
                }
            }
        ...
        if self.config.enable_swarm {
            ...
            if let Some(ref swarm) = *swarm_guard {
                ...
                for attempt in 0..self.config.max_retries {
                    match timeout(
                        fetch_timeout,
                        self.get_round_from_swarm(swarm_clone.as_ref(), round_number),
                    )
                    .await
                    {
                        Ok(Ok(round)) => {
                            if let Err(e) = self.verify_round(&round).await {
                                warn!(
                                    "Swarm round {} failed verification (attempt {}): {}",
                                    round_number,
                                    attempt + 1,
                                    e
                                );
                        ...
                        Ok(Err(e)) => {
                            warn!(
                                "Round {} not found in swarm (attempt {}): {}",
                                round_number,
                                attempt + 1,
                                e
                            );
                        ...
                        Err(_) => {
                            warn!(
                                "Swarm fetch for round {} timed out (attempt {})",
                                round_number,
                                attempt + 1
                            );
                        }
        ...
        if self.config.enable_ipfs {
            ...
            if let Some(ref ipfs) = *ipfs_guard {
                ...
                for attempt in 0..self.config.max_retries {
                    match timeout(
                        fetch_timeout,
                        self.get_round_from_ipfs(ipfs_clone.as_ref(), round_number),
                    )
                    .await
                    {
                        Ok(Ok(round)) => {
                            if let Err(e) = self.verify_round(&round).await {
                                warn!(
                                    "IPFS round {} failed verification (attempt {}): {}",
                                    round_number,
                                    attempt + 1,
                                    e
                                );
                        ...
                        Ok(Err(e)) => {
                            warn!(
                                "Round {} not found in IPFS (attempt {}): {}",
                                round_number,
                                attempt + 1,
                                e
                            );
                        ...
                        Err(_) => {
                            warn!(
                                "IPFS fetch for round {} timed out (attempt {})",
                                round_number,
                                attempt + 1
                            );
                        }
        ...
        debug!(
            "🔄 Falling back to direct drand API for round {}",
            round_number
        );
        ...
        for attempt in 0..self.config.max_retries {
            match timeout(
                fetch_timeout,
                self.drand_client.fetch_range(round_number, round_number),
            )
            .await
            {
                Ok(Ok(rounds)) => {
                    if let Some(round) = rounds.first() {
                        ...
                        if let Err(e) = self.verify_round(round).await {
                            error!(
                                "🚨 SECURITY: drand round {} failed verification: {}",
                                round_number, e
                            );
                ...
                Ok(Err(e)) => {
                    warn!(
                        "Failed to fetch round {} from drand (attempt {}): {}",
                        round_number,
                        attempt + 1,
                        e
                    );
                ...
                Err(_) => {
                    warn!(
                        "Drand API fetch for round {} timed out (attempt {})",
                        round_number,
                        attempt + 1
                    );
                }
    ...
    async fn verify_round(&self, round: &DrandRound) -> Result<(), DrandError> {
        ...
        let entropy_score =
            verify_entropy_quality(&round.randomness, self.config.min_entropy_threshold)?;
        ...
        debug!(
            "✅ Round {} verified: entropy score = {:.2} bits/byte",
            round.round, entropy_score
        );
        Ok(())
    }
    ...
    async fn get_round_from_cache(
        &self,
        cache: &dyn EntropyCacheInterface,
        round_number: u64,
    ) -> Result<DrandRound, DrandError> {
        let block = cache
            .get_block(round_number)
            .await
        ...
        Err(DrandError::NetworkError(format!(
            "Round {} not found in block {}-{}",
            round_number, block.start_round, block.end_round
        )))
    }

    /// Get a round from swarm
    async fn get_round_from_swarm(
        &self,
        swarm: &dyn EntropySwarmInterface,
        round_number: u64,
    ) -> Result<DrandRound, DrandError> {
        let block = swarm
            .get_block(round_number)
    ...
    async fn get_round_from_ipfs(
        &self,
        ipfs: &dyn IpfsInterface,
        round_number: u64,
    ) -> Result<DrandRound, DrandError> {
        let block = ipfs
            .get_block(round_number)
...
mod tests {
    ...
    fn test_min_entropy_estimation() {
        ...
        assert!(
            score > 4.0,
            "Good entropy should have high min-entropy score: {}",
            score
        );
        ...
        assert!(
            bad_score < 1.0,
            "All-zero data should have very low min-entropy: {}",
            bad_score
        );
    }
    ...
    fn test_block_integrity_verification() {
        ...
        assert!(
            verify_block_integrity(&block).is_ok(),
            "Block with correct hash should verify"
        );
    ...
    fn test_constant_time_eq() {
        ...
        assert!(
            ...
            "Different length arrays should not match"
        );
    }
