...
fn benchmark_handshake_compute(c: &mut Criterion) {
    ...
    group.bench_function("client-handshake-compute", |b| {
        b.iter(|| {
            ...
            let sig =
                MlDsa::sign(&ephemeral.public_key, alice_identity.signing_key()).expect("sign");
    ...
    group.bench_function("server-handshake-compute", |b| {
        b.iter(|| {
            ...
            // Sign our response
            let response_data = [
                our_ephemeral.public_key.as_slice(),
                encaps.ciphertext.as_slice(),
            ]
            .concat();
    ...
    group.bench_function("full-handshake-both-sides", |b| {
        b.iter(|| {
            ...
            let alice_sig =
                MlDsa::sign(&alice_eph.public_key, alice_identity.signing_key()).expect("sign");

            // Bob: Respond
            let bob_encaps = MlKem::encapsulate(&alice_eph.public_key).expect("encaps");
            let bob_eph = MlKem::generate_keypair().expect("keygen");
            let bob_response = [
                bob_eph.public_key.as_slice(),
                bob_encaps.ciphertext.as_slice(),
            ]
            .concat();
            ...
            let alice_ss =
                MlKem::decapsulate(&bob_encaps.ciphertext, alice_eph.secret_key()).expect("decaps");
            ...
            // Bob: Decapsulate
            let bob_ss =
                MlKem::decapsulate(&alice_encaps.ciphertext, bob_eph.secret_key()).expect("decaps");
...
fn benchmark_paper_claims(c: &mut Criterion) {
    ...
    group.bench_function("3-hop-circuit-crypto-only", |b| {
        b.iter(|| {
            ...
            let _guard_ss = MlKem::decapsulate(&guard_encaps.ciphertext, guard_eph.secret_key())
                .expect("decaps");
            ...
            let _middle_ss = MlKem::decapsulate(&middle_encaps.ciphertext, middle_eph.secret_key())
                .expect("decaps");
            ...
            let _exit_ss =
                MlKem::decapsulate(&exit_encaps.ciphertext, exit_eph.secret_key()).expect("decaps");
