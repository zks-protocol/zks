...
use zks_wire::{Peer, PeerId, Swarm, SwarmCircuit};
...
async fn test_simple_circuit(swarm: &Swarm) -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        all_peers.len(),
        unique_peers.len(),
        "Circuit contains duplicate peers"
    );
...
async fn test_circuit_builder(swarm: &Swarm) -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!(
        "Built circuit with {} hops (requested 2-4)",
        circuit.hop_count()
    );
    ...
    info!(
        "Built circuit with {} hops (requested 5-6)",
        circuit.hop_count()
    );
    ...
    let circuit = swarm.build_circuit(3, 3).await?;
    info!(
        "Built circuit with {} hops (requested exactly 3)",
        circuit.hop_count()
    );
...
async fn test_large_circuit(swarm: &Swarm) -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        decrypted, test_data,
        "Large circuit encryption/decryption failed"
    );
