...
impl RngCore for TrueEntropyRngCompat {
    ...
    fn fill_bytes(&mut self, dest: &mut [u8]) {
        ...
        getrandom::getrandom(dest).expect("CRITICAL: OS RNG failed - cannot generate secure keys");
...
impl RngCore for TrueEntropyRngWithExternal {
    ...
    fn fill_bytes(&mut self, dest: &mut [u8]) {
        ...
        let pos = self
            .position
            .fetch_add(dest.len(), std::sync::atomic::Ordering::SeqCst);
...
impl MlKem {
    ...
    fn validate_polynomial_coefficients(public_key: &[u8]) -> Result<()> {
        ...
        tracing::debug!(
            "✅ All {} polynomial coefficients validated (< q={})",
            NUM_POLYS * 256,
            Q
        );
    ...
    pub fn decapsulate(ciphertext: &[u8], secret_key: &[u8]) -> Result<Zeroizing<Vec<u8>>> {
        ...
        let shared_secret = dk
            .decapsulate(ciphertext.try_into().unwrap())
...
mod tests {
    ...
    fn test_encapsulation_decapsulation() {
        ...
        assert_eq!(
            encapsulation.shared_secret.as_ref() as &[u8],
            shared_secret_bob.as_ref() as &[u8]
        );
    }
    ...
    fn test_invalid_key_sizes() {
        ...
        match result {
            Err(PqcError::InvalidKey(msg)) => {
                assert!(
                    msg.contains("Invalid public key size"),
        ...
        match result {
            Err(PqcError::InvalidInput(msg)) => {
                assert!(
                    msg.contains("Invalid ciphertext size"),
                    "Error should mention invalid ciphertext: {}",
                    msg
                );
            }
        ...
        match result {
            Err(PqcError::InvalidKey(msg)) => {
                assert!(
                    msg.contains("Invalid secret key size"),
                    "Error should mention invalid secret key: {}",
                    msg
                );
            }
            Err(other) => panic!("Expected InvalidKey error, got: {:?}", other),
    ...
    fn test_session_key_derivation() {
        ...
        let session_key = MlKem::derive_session_key(&shared_secret, Some(salt), Some(info), 32)
            .expect("Key derivation should succeed");

        assert_eq!(session_key.len(), 32);

        // Test that same inputs produce same output
        let session_key2 = MlKem::derive_session_key(&shared_secret, Some(salt), Some(info), 32)
            .expect("Key derivation should succeed");

        assert_eq!(
            session_key.as_ref() as &[u8],
            session_key2.as_ref() as &[u8]
        );
    }
    ...
    fn test_session_key_derivation_with_defaults() {
        ...
        let session_key1 = MlKem::derive_session_key(&shared_secret, None, None, 32)
            .expect("Key derivation with defaults should succeed");

        let session_key2 = MlKem::derive_session_key(&shared_secret, None, None, 32)
            .expect("Key derivation with defaults should succeed");

        // Same inputs should produce same output
        assert_eq!(
            session_key1.as_ref() as &[u8],
            session_key2.as_ref() as &[u8]
        );

        // Different from empty salt/info
        let session_key_empty = MlKem::derive_session_key(&shared_secret, Some(b""), Some(b""), 32);
    ...
    fn test_public_key_validation_low_entropy() {
        ...
        match result {
            Err(PqcError::InvalidKey(msg)) => {
                assert!(
                    msg.contains("entropy") || msg.contains("unique bytes"),
                    "Error should mention low entropy: {}",
                    msg
                );
            }
    ...
    fn test_public_key_validation_repeating_pattern() {
        ...
        match result {
            Err(PqcError::InvalidKey(msg)) => {
                assert!(
                    msg.contains("repeating"),
                    "Error should mention repeating pattern: {}",
                    msg
                );
            }
    ...
    fn test_polynomial_coefficient_validation() {
        ...
        assert!(
            result.is_ok(),
            "Valid keypair public key should pass validation: {:?}",
            result
        );
    }
}
