...
fn benchmark_mlkem_comparative(c: &mut Criterion) {
    ...
    group.bench_function("ML-KEM-768/Decaps", |b| {
        b.iter(|| {
            let ss =
                MlKem::decapsulate(black_box(&enc768.ciphertext), black_box(kp768.secret_key()));
            black_box(ss)
        })
    });
    ...
    group.bench_function("ML-KEM-1024/Encaps", |b| {
        b.iter(|| {
            let (ct1, es, ss) =
                incremental_mlkem::encaps1(black_box(&keys1024.hdr)).expect("encaps1");
            let ct2 = incremental_mlkem::encaps2(black_box(&keys1024.ek), black_box(&es))
                .expect("encaps2");
    ...
    group.bench_function("ML-KEM-1024/Decaps", |b| {
        b.iter(|| {
            let ss = incremental_mlkem::decaps(
                black_box(&keys1024.dk),
                black_box(&ct1),
                black_box(&ct2),
            );
            black_box(ss)
...
fn benchmark_pq_overhead(c: &mut Criterion) {
    ...
    group.bench_function("ML-KEM-1024/FullKeyExchange", |b| {
        b.iter(|| {
            ...
            // Server encapsulates
            let (ct1, es, server_ss) =
                incremental_mlkem::encaps1(&client_keys.hdr).expect("encaps1");
    ...
    group.bench_function("Authenticated-KE/ML-KEM-1024+ML-DSA-87", |b| {
        b.iter(|| {
            ...
            let alice_sig =
                MlDsa::sign(&alice_eph.hdr, alice_identity.signing_key()).expect("sign");

            // Bob verifies and encapsulates
            MlDsa::verify(&alice_eph.hdr, &alice_sig, alice_identity.verifying_key())
                .expect("verify");
