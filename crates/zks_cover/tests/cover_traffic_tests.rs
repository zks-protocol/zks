...
async fn test_cover_indistinguishable() {
    ...
    for _ in 0..10 {
        let msg = generator
            .generate_cover(Some("test_circuit".to_string()))
            .await
    ...
    for msg in &messages {
        ...
        assert!(
            ...
            "Cover payload should not be all zeros"
        );
    }
...
async fn test_timing_distribution() {
    ...
    let generator =
        Arc::new(CoverGenerator::new(config.clone()).expect("Failed to create generator"));
    let scheduler = CoverScheduler::new(config, generator).expect("Failed to create scheduler");

    // Generate burst of covers
    let start = Instant::now();
    let covers = scheduler
        .schedule_burst(10, Some("timing_test".to_string()))
        .await
    ...
    assert!(
        ...
        "Burst scheduling should be fast"
    );
    ...
    for cover in &covers {
        assert_eq!(
            cover.payload.len(),
            512,
            "Each cover should have 512-byte payload"
        );
        assert_eq!(
            cover.circuit_id,
            Some("timing_test".to_string()),
            "Circuit ID should match"
        );
    }
}
...
async fn test_scheduler_integration() {
    ...
    let covers = scheduler
        .schedule_burst(5, Some("integration_circuit".to_string()))
        .await
        .expect("Failed to schedule burst");
    ...
    for cover in &covers {
        assert_eq!(
            cover.circuit_id,
            Some("integration_circuit".to_string()),
...
async fn test_poisson_realism() {
    ...
    assert!(
        average >= 3.0 && average <= 7.0,
        "Average covers per batch should be around 5, got {}",
        average
    );
}
...
async fn test_config_diversity() {
    ...
    let low_covers = low_generator
        .generate_covers(5, Some("low_circuit".to_string()))
        .await
        .expect("Failed to generate low covers");
    let high_covers = high_generator
        .generate_covers(5, Some("high_circuit".to_string()))
        .await
    ...
    for cover in low_covers.iter().chain(high_covers.iter()) {
        assert_eq!(
            cover.payload.len(),
            512,
            "All covers should have 512-byte payload"
        );
        assert!(
            cover.circuit_id.is_some(),
            "All covers should have circuit ID"
        );
    }
}
...
async fn test_scheduler_timing() {
    ...
    let start = Instant::now();
    let covers = scheduler
        .schedule_poisson(Some("timing_circuit".to_string()))
        .await
        .expect("Failed to schedule Poisson covers");
...
async fn test_generation_performance() {
    ...
    let covers = generator
        .generate_covers(100, Some("perf_circuit".to_string()))
        .await
