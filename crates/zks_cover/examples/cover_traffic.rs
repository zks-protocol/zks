...
use zks_cover::{
    CoverConfig, CoverGenerator, CoverMessage, CoverScheduler, CoverTransport, CoverType,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let cover = generator
        .generate_cover(Some("circuit-001".to_string()))
        .await?;
    ...
    scheduler
        .start(tx, Some("scheduled-circuit".to_string()))
        .await?;
    ...
    let timeout = tokio::time::timeout(Duration::from_secs(5), async {
        while received < 3 {
            if let Some(msg) = rx.recv().await {
                received += 1;
                println!(
                    "   Received scheduled cover {}: {:?}",
                    received, msg.cover_type
                );
            }
        }
    })
