...
fn bench_cover_generation(c: &mut Criterion) {
    ...
    let generator = runtime
        .block_on(async { CoverGenerator::new(config).expect("Failed to create generator") });

    c.bench_function("cover_generation_single", |b| {
        b.iter(|| {
            runtime.block_on(async {
                generator
                    .generate_cover(black_box(Some("bench_circuit".to_string())))
                    .await
            })
        });
    });
    ...
    c.bench_function("cover_generation_no_circuit", |b| {
        b.iter(|| runtime.block_on(async { generator.generate_cover(black_box(None)).await }));
    });
}
...
fn bench_cover_batch_generation(c: &mut Criterion) {
    ...
    c.bench_function("cover_batch_5", |b| {
        b.iter(|| {
            runtime.block_on(async {
                generator
                    .generate_batch_cover(
                        black_box(5),
                        black_box(Some("bench_circuit".to_string())),
                    )
                    .await
            })
        });
...
fn bench_cover_scheduler(c: &mut Criterion) {
    ...
    c.bench_function("scheduler_next_time", |b| {
        b.iter(|| scheduler.next_cover_time());
    });
...
fn bench_memory_patterns(c: &mut Criterion) {
    ...
    c.bench_function("cover_with_zeroize", |b| {
        b.iter(|| {
            runtime.block_on(async {
                let msg = generator
                    .generate_cover(Some("bench_circuit".to_string()))
                    .await
