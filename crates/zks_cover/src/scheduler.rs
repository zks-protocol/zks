...
impl CoverScheduler {
    ...
    pub async fn schedule_scenario(
        &self,
        scenario: CoverScenario,
        circuit_id: Option<String>,
    ) -> Result<Vec<CoverMessage>> {
        self.generator
            .generate_scenario_covers(scenario, circuit_id)
            .await
    }
    ...
    pub fn update_rate(&mut self, new_rate: f64) -> Result<()> {
        if new_rate <= 0.0 {
            return Err(CoverError::InvalidConfig(
...
impl AdaptiveCoverScheduler {
    ...
    pub async fn schedule_scenario(
        ...
        self.base_scheduler
            .schedule_scenario(scenario, circuit_id)
            .await
    }
}
...
impl CoverSchedulerBuilder {
    ...
    pub fn build(self) -> Result<CoverScheduler> {
        let config = self
            .config
            .ok_or_else(|| CoverError::InvalidConfig("Configuration required".to_string()))?;
        let generator = self
            .generator
            .ok_or_else(|| CoverError::InvalidConfig("Generator required".to_string()))?;
...
mod tests {
    ...
    async fn test_poisson_scheduling() {
        ...
        assert_eq!(
            covers.len(),
            3,
            "Burst scheduling should generate exactly 3 covers"
        );
    ...
    async fn test_scenario_scheduling() {
        ...
        let low_covers = scheduler
            .schedule_scenario(CoverScenario::LowTraffic, None)
            .await
            .unwrap();
        assert_eq!(low_covers.len(), 1);

        let medium_covers = scheduler
            .schedule_scenario(CoverScenario::MediumTraffic, None)
            .await
            .unwrap();
