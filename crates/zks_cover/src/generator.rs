...
impl CoverGenerator {
    ...
    pub async fn generate_cover(&self, circuit_id: Option<String>) -> Result<CoverMessage> {
        ...
        let cover_message = CoverMessage::new(cover_type, encrypted_payload, circuit_id);
    ...
    pub async fn generate_covers(
        &self,
        count: usize,
        circuit_id: Option<String>,
    ) -> Result<Vec<CoverMessage>> {
    ...
    pub async fn generate_poisson_covers(
        &self,
        circuit_id: Option<String>,
    ) -> Result<Vec<CoverMessage>> {
        let mut rng = rand::rngs::StdRng::from_entropy();
    ...
    pub async fn generate_scenario_covers(
        &self,
    ...
    pub fn validate_cover(&self, cover: &CoverMessage) -> Result<bool> {
        ...
        if !matches!(
            cover.cover_type,
            CoverType::Regular | CoverType::Loop | CoverType::Drop
        ) {
            return Ok(false);
        }
    ...
    async fn encrypt_payload(&self, payload: &[u8]) -> Result<Vec<u8>> {
        if self.config.use_post_quantum() {
            ...
            let mut cipher = WasifVernam::new(key).map_err(|e| {
                CoverError::EncryptionError(format!("Failed to create cipher: {:?}", e))
            })?;
...
mod tests {
    ...
    async fn test_scenario_generation() {
        ...
        let low_covers = generator
            .generate_scenario_covers(CoverScenario::LowTraffic, None)
            .await
            .unwrap();
        assert_eq!(low_covers.len(), 1);

        let medium_covers = generator
            .generate_scenario_covers(CoverScenario::MediumTraffic, None)
            .await
            .unwrap();
        assert_eq!(medium_covers.len(), 3);

        let high_covers = generator
            .generate_scenario_covers(CoverScenario::HighTraffic, None)
            .await
            .unwrap();
    ...
    async fn test_builder() {
        let config = CoverConfig::default();
        let generator = CoverGeneratorBuilder::new().config(config).build().unwrap();
