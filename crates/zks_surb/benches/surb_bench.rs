...
use zks_surb::{MemorySurbStorage, SurbConfig, SurbEncryption, SurbStorage, ZksSurb};
...
fn bench_surb_creation(c: &mut Criterion) {
    ...
    c.bench_function("surb_create", |b| {
        b.iter(|| runtime.block_on(async { ZksSurb::create(black_box(&pk)) }));
    });
    ...
    c.bench_function("surb_create_with_config", |b| {
        b.iter(|| {
            runtime
                .block_on(async { ZksSurb::create_with_config(black_box(&pk), black_box(&config)) })
        });
    });
}
...
fn bench_surb_encryption(c: &mut Criterion) {
    ...
    let pk = generate_test_ml_kem_public_key();
    let surb = runtime.block_on(async { ZksSurb::create(&pk).expect("Failed to create SURB") });

    let encryption = SurbEncryption::from_surb(&surb).expect("Failed to create encryption");
    ...
    for (i, payload) in test_payloads.iter().enumerate() {
        ...
        c.bench_function(&format!("surb_encrypt_{}b", size), |b| {
            b.iter(|| encryption.encrypt(black_box(payload)))
        });
    }
}
...
fn bench_surb_decryption(c: &mut Criterion) {
    ...
    let test_payloads = vec![vec![0u8; 32], vec![0u8; 256], vec![0u8; 1024]];

    let encrypted_payloads: Vec<_> = test_payloads
        .iter()
    ...
    for (i, encrypted) in encrypted_payloads.iter().enumerate() {
        ...
        c.bench_function(&format!("surb_decrypt_{}b", size), |b| {
            b.iter(|| encryption.decrypt(black_box(encrypted)))
        });
    }
}
...
fn bench_surb_storage(c: &mut Criterion) {
    ...
    let surbs: Vec<_> = (0..10)
        .map(|_| runtime.block_on(async { ZksSurb::create(&pk).expect("Failed to create SURB") }))
        .collect();

    c.bench_function("surb_storage_store", |b| {
        b.iter(|| {
            let surb = surbs[0].clone();
            runtime.block_on(async { storage.store_surb(black_box(surb)) })
        });
    });
    ...
    c.bench_function("surb_storage_get", |b| {
        runtime.block_on(async {
            storage
                .store_surb(surbs[1].clone())
                .await
                .expect("Failed to store SURB");
        });

        b.iter(|| runtime.block_on(async { storage.get_surb(black_box(surbs[1].id())) }));
    });

    c.bench_function("surb_storage_has", |b| {
        runtime.block_on(async {
            storage
                .store_surb(surbs[2].clone())
                .await
                .expect("Failed to store SURB");
        });

        b.iter(|| runtime.block_on(async { storage.has_surb(black_box(surbs[2].id())) }));
    });

    c.bench_function("surb_storage_count", |b| {
        b.iter(|| runtime.block_on(async { storage.count() }));
    });
}
    ...
    c.bench_function("surb_complete_workflow", |b| {
        b.iter(|| {
            runtime.block_on(async {
                ...
                // 2. Create encryption
                let encryption =
                    SurbEncryption::from_surb(&surb).expect("Failed to create encryption");

                // 3. Encrypt message
                let message = b"Anonymous reply message";
                let encrypted = encryption.encrypt(message).expect("Failed to encrypt");
