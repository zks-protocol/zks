...
use zks_surb::{surb_utils, ReplyRequest, SurbConfig, ZksSurb};
...
async fn test_surb_with_config() {
    ...
    assert!(
        !surb.route_header.is_empty(),
        "Route header should not be empty"
    );
    assert!(
        surb.route_header.len() > 100,
        "Route header should be substantial for 5 hops"
    );
}
