...
async fn test_surb_roundtrip() {
    ...
    let deserialized: ZksSurb =
        serde_json::from_str(&serialized).expect("Failed to deserialize SURB");
...
async fn test_anonymous_reply_flow() {
    ...
    let encrypted_reply = encryption
        .encrypt(reply_content)
    ...
    let decrypted_reply = alice_encryption
...
async fn test_surb_storage() {
    ...
    assert!(!storage
        .has_surb(&id1)
        .await
        .expect("Failed to check SURB 1"));

    // Store SURBs
    storage
        .store_surb(surb1.clone())
        .await
        .expect("Failed to store SURB 1");
    storage
        .store_surb(surb2.clone())
        .await
        .expect("Failed to store SURB 2");
    ...
    storage
        .remove_surb(&id1)
        .await
        .expect("Failed to remove SURB 1");
...
async fn test_surb_lifetime() {
    ...
    let (surb, _private_data) =
        ZksSurb::create_with_config(&pk, &config).expect("Failed to create SURB with config");
...
async fn test_encryption_various_sizes() {
    ...
    for payload in test_payloads {
        let encrypted = encryption.encrypt(&payload).expect(&format!(
            "Failed to encrypt payload of size {}",
            payload.len()
        ));

        let decrypted = encryption.decrypt(&encrypted).expect(&format!(
            "Failed to decrypt payload of size {}",
            payload.len()
        ));

        assert_eq!(
            decrypted, payload,
            "Decrypted payload doesn't match original"
        );
    }
}
...
async fn test_concurrent_surb_operations() {
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            let (surb, _private_data) =
                ZksSurb::create(&pk).expect(&format!("Failed to create SURB {}", i));
            storage
                .store_surb(surb.clone())
                .await
