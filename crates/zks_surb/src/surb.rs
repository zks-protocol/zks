...
impl ZksSurb {
    ...
    async fn generate_route_header(
        ...
        let route_header = bincode::serialize(&route_hops).map_err(|e| {
            SurbError::SerializationError(format!("Failed to serialize route: {}", e))
        })?;
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self> {
        bincode::deserialize(bytes).map_err(|e| {
            SurbError::SerializationError(format!("Failed to deserialize SURB: {}", e))
        })
    }
    ...
    pub fn from_base64(b64: &str) -> Result<Self> {
        let bytes = general_purpose::STANDARD.decode(b64).map_err(|e| {
            SurbError::SerializationError(format!("Failed to decode base64: {}", e))
        })?;
...
impl ReplyRequest {
    ...
    pub fn from_surb(surb: ZksSurb, content: &[u8]) -> Result<Self> {
        ...
        if content.len() > 1024 {
            // Max reply size limit
            return Err(SurbError::InvalidConfig(
                "Reply content too large".to_string(),
            ));
        }
