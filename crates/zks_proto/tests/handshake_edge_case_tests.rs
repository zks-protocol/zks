...
use zks_proto::handshake::{
    Handshake, HandshakeFinish, HandshakeInit, HandshakeResponse, HandshakeRole, HandshakeState,
};
...
fn test_invalid_trusted_responder_key_size() {
    ...
    assert!(
        result.is_err(),
        "Should fail with invalid trusted responder key size"
    );
}
...
fn test_invalid_state_transitions() {
    ...
    assert!(
        result.is_err(),
        "Initiator should not be able to create response"
    );
    ...
    assert!(
        result.is_err(),
        "Should fail with invalid state for processing init"
    );
}
...
fn test_invalid_ephemeral_key_sizes() {
    ...
    if result.is_ok() {
        ...
        assert!(
            ...
            "Should fail when creating response with invalid ephemeral key"
        );
    }
}
...
fn test_invalid_signing_public_key_size() {
    ...
    assert!(
        result.is_err(),
        "Should fail with invalid signing public key size"
    );
}
...
fn test_mismatched_trusted_responder_key() {
    ...
    let mut responder = Handshake::new_responder("test_room".to_string());
    responder
        .process_init(&initiator.create_init().unwrap())
        .unwrap();
    ...
    assert!(
        result.is_err(),
        "Should fail with mismatched trusted responder key"
    );
}
...
fn test_concurrent_handshake_attempts() {
    ...
    let mut initiator1 =
        Handshake::new_initiator("test_room".to_string(), trusted_key.clone()).unwrap();
    ...
    assert!(
        result2.is_err(),
        "Second init should fail when responder is in wrong state"
    );
}
...
fn test_memory_cleanup_after_failure() {
    ...
    assert!(
        initiator.shared_secret().is_none(),
        "Shared secret should remain None after failure"
    );
}
...
fn test_transcript_hash_consistency() {
    ...
    assert!(
        result.is_ok(),
        "Handshake should succeed with consistent transcript hashes"
    );
}
