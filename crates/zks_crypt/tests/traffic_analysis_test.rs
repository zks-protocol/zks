...
fn test_traffic_size_obfuscation() {
    ...
    for size in test_sizes {
        ...
        assert_eq!(
            encrypted.len(),
            expected_size,
            "Encrypted size should be plaintext size + {} bytes",
            36 * circuit.layer_keys.len()
        );
...
fn test_traffic_pattern_randomness() {
    ...
    assert!(
        pattern.entropy > 5.5,
        "Encrypted traffic should have good entropy (got {})",
        pattern.entropy
    );
    assert!(
        pattern.is_random_like,
        "Encrypted traffic should appear random-like"
    );

    // Compare with plaintext pattern
    let plaintext_pattern = circuit.analyze_traffic_pattern(plaintext);
    assert!(
        pattern.entropy > plaintext_pattern.entropy,
...
fn test_identical_plaintext_obfuscation() {
    ...
    assert_eq!(
        ...
        "Same plaintext produces same ciphertext with fixed keys (limitation)"
    );
    ...
    assert!(
        pattern1.entropy > 5.5,
        "First encryption should have good entropy"
    );
    assert!(
        pattern2.entropy > 5.5,
        "Second encryption should have good entropy"
    );
    assert!(
        (pattern1.entropy - pattern2.entropy).abs() < 0.0001,
        "Both patterns should have very similar entropy"
    );
}
...
fn test_same_size_different_content() {
    ...
    assert!(
        pattern2.entropy > 5.5,
        "Pattern2 entropy {} should be > 5.5",
        pattern2.entropy
    );
    ...
    assert!(
        entropy_diff_1_2 < 0.5,
        "Entropy difference between similar-sized plaintexts should be small"
    );
    assert!(
        entropy_diff_2_3 < 0.5,
        "Entropy difference between similar-sized plaintexts should be small"
    );
}
...
fn test_multi_layer_obfuscation() {
    ...
    assert!(
        single_pattern.entropy > 4.5,
        "Single layer entropy {} should be > 4.5",
        single_pattern.entropy
    );
    assert!(
        multi_pattern.entropy > 5.5,
        "Multi layer entropy {} should be > 5.5",
        multi_pattern.entropy
    );

    // Multi-layer should have slightly better entropy (though both are good)
    assert!(
        multi_pattern.entropy >= single_pattern.entropy,
...
fn test_timing_analysis() {
    ...
    assert!(
        ...
        "Timing deviation should be less than 1000% (got {}%)",
...
fn test_volume_analysis() {
    ...
    println!(
        "Small message: {} bytes -> {} bytes encrypted",
        small_message.len(),
        encrypted_small.len()
    );
    println!(
        "Medium message: {} bytes -> {} bytes encrypted",
        medium_message.len(),
        encrypted_medium.len()
    );
    println!(
        "Large message: {} bytes -> {} bytes encrypted",
        large_message.len(),
        encrypted_large.len()
    );
    ...
    assert!(
        small_pattern.entropy > 5.0,
        "Small messages should still have reasonable entropy"
    );
    assert!(
        medium_pattern.entropy > 5.5,
        "Medium messages should have good entropy"
    );
    assert!(
        large_pattern.entropy > 5.5,
        "Large messages should have good entropy"
    );
}
...
fn test_correlation_analysis() {
    ...
    assert!(
        correlation.abs() < 0.1,
        "Correlation between plaintext and ciphertext should be very low (got {})",
        correlation
    );
    ...
    assert!(
        correlation2.abs() < 0.1,
        "Correlation between different ciphertexts should also be low"
    );
}
