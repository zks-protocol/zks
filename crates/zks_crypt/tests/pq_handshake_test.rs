...
fn test_ml_kem_key_generation() {
    ...
    assert_eq!(
        ...
        1568,
        "Public key should be 1568 bytes"
    );
    ...
    assert_eq!(
        ...
        3168,
        "Secret key should be 3168 bytes"
    );

    // Keys should not be all zeros (sanity check)
    assert!(
        !keypair.public_key.iter().all(|&b| b == 0),
...
fn test_ml_kem_encapsulation_decapsulation() {
    ...
    let bob_encapsulation = MlKem::encapsulate(&alice_public_key).expect("Failed to encapsulate");

    // Verify encapsulation produces correct sizes
    assert_eq!(
        bob_encapsulation.ciphertext.len(),
        1568,
        "Ciphertext should be 1568 bytes (ML-KEM-1024)"
    );
    assert_eq!(
        bob_encapsulation.shared_secret.len(),
        32,
...
fn test_ml_kem_shared_secret_randomness() {
    ...
    for _ in 0..10 {
        ...
        let encapsulation = MlKem::encapsulate(&keypair.public_key).expect("Failed to encapsulate");
    ...
    assert!(
        (ratio - 0.5).abs() < 0.2,
        "Shared secret should appear random: {:.4}",
        ratio
    );
}
...
fn test_post_quantum_handshake_integration() {
    ...
    let alice_shared_secret =
        MlKem::decapsulate(&bob_encapsulation.ciphertext, alice_keypair.secret_key())
            .expect("Failed to decapsulate");
    ...
    let encrypted = alice_cipher
        .encrypt_sequenced(message)
        .expect("Encryption failed");
    let decrypted = bob_cipher
        .decrypt_sequenced(&encrypted)
        .expect("Decryption failed");

    assert_eq!(
        decrypted,
        message.to_vec(),
        "Post-quantum encrypted communication should work"
    );
}
...
fn test_ml_kem_performance_baseline() {
    ...
    assert!(
        encaps_time.as_millis() < 100,
        "Encapsulation should complete within 100ms"
    );
    assert!(
        decaps_time.as_millis() < 100,
        "Decapsulation should complete within 100ms"
    );
...
fn test_post_quantum_forward_secrecy() {
    ...
    // First session
    let encapsulation1 =
        MlKem::encapsulate(&keypair1.public_key).expect("Failed to encapsulate first session");
    ...
    // Second session
    let encapsulation2 =
        MlKem::encapsulate(&keypair2.public_key).expect("Failed to encapsulate second session");
