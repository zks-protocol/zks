...
fn test_ciphertext_uniformity() {
    ...
    assert!(
        (ratio - 0.5).abs() < 0.01,
        "Ciphertext is not uniformly random: {:.4}",
        ratio
    );
}
...
fn test_statistical_independence() {
    ...
    assert!(
        correlation.abs() < 0.1,
        "Strong correlation detected between plaintext and ciphertext: {}",
        correlation
    );
}
...
fn test_message_size_handling() {
    ...
    for size in sizes {
        ...
        assert_eq!(
            decrypted, plaintext,
            "Failed for message size {}: decrypted != original",
            size
        );
    }
}
...
fn test_key_sensitivity() {
    ...
    assert!(
        difference_ratio > 0.3, // At least 30% of bytes should differ
        "Key sensitivity failed: only {}% of bytes differed",
...
fn test_true_vernam_perfect_secrecy() {
    ...
    assert!(
        (ratio - 0.5).abs() < 0.15,
        "Ciphertext should appear reasonably random in TRUE Vernam mode: {:.4}",
        ratio
    );
}
