...
impl MockOnionCircuit {
    ...
    fn create_layered_payload(&mut self, plaintext: &[u8]) -> Vec<u8> {
        ...
        for i in (0..self.nodes.len()).rev() {
            ...
            let next_hop = if i == self.nodes.len() - 1 {
                255
            } else {
                (i + 1) as u8
            };
    ...
    fn check_correlation_vulnerability(
        &mut self,
        plaintext1: &[u8],
        plaintext2: &[u8],
    ) -> CorrelationAnalysis {
        ...
        for i in 0..self.nodes.len() {
            ...
            let size_correlation =
                if (visibility1.data_size as i32 - visibility2.data_size as i32).abs() <= 3 {
                    1.0
                } else {
                    0.0
                };
...
fn test_single_hop_anonymity() {
    ...
    assert!(
        first_hop_visibility.can_decrypt,
        "First hop should be able to decrypt one layer"
    );
    assert!(
        first_hop_visibility.entropy > 5.5,
        "First hop should see high entropy data"
    );
    assert!(
        first_hop_visibility.data_size > plaintext.len(),
        "First hop should see larger data due to onion layers"
    );
    assert!(
        first_hop_visibility.can_see_next,
        "First hop should know there are more hops"
    );
    assert!(
        !first_hop_visibility.can_see_previous,
        "First hop should not see previous hops"
    );
}
...
fn test_multi_hop_anonymity_chain() {
    ...
    for i in 0..circuit.nodes.len() {
        ...
        // Debug output
        println!(
            "Hop {}: entropy = {:.3}, data_size = {}",
            i, visibility.entropy, visibility.data_size
        );
        ...
        if i == 0 {
            assert!(
                !visibility.can_see_previous,
                "First hop should not see previous"
            );
            ...
            assert!(
                visibility.can_see_previous,
                "Middle hop should see previous"
            );
        ...
        if i == circuit.nodes.len() - 1 {
            ...
            assert!(
                visibility.entropy > 2.0,
                "Exit hop should see readable data with some entropy"
            );
        } else {
            // Entry and middle hops see encrypted layers
            assert!(
                visibility.entropy > 4.0,
                "Hop {} should see high entropy encrypted data",
                i
            );
        }
    }
...
fn test_traffic_correlation_resistance() {
    ...
    for correlation in &analysis.hop_correlations {
        println!(
            "Hop {}: size_correlation={}, entropy_diff={}, can_correlate={}",
            correlation.hop_id,
            correlation.size_correlation,
            correlation.entropy_diff,
            correlation.can_correlate
        );
    }
    ...
    assert!(
        analysis.vulnerable_hops <= 4,
        "All hops should not be able to correlate messages"
    );
    ...
    for correlation in &analysis.hop_correlations {
        ...
        assert!(
            correlation.entropy_diff >= 0.0,
            "Hop {} should have some entropy difference",
            correlation.hop_id
        );
    }
}
...
fn test_same_message_correlation() {
    ...
    assert!(
        analysis.vulnerable_hops <= 3,
        "Correlation should be limited across hops"
    );
}
...
fn test_hop_isolation() {
    ...
    assert!(
        ...
        "Should not decrypt properly without middle hop"
    );
}
...
fn test_size_obfuscation_across_hops() {
    ...
    for i in 0..circuit.nodes.len() {
        ...
        if let Some(prev_size) = previous_size {
            ...
            assert_ne!(
                visibility.data_size, prev_size,
                "Hop {} should see different size than previous hop",
                i
            );
        }
        ...
        assert!(
            visibility.entropy > 3.5,
            "Hop {} should see reasonable entropy",
            i
        );
    }
}
...
fn test_entry_node_privacy() {
    ...
    assert!(
        size_diff <= 20,
        "Entry node should see roughly similar size for different destinations (diff: {})",
        size_diff
    );
    ...
    assert!(
        entropy_diff < 0.5,
        "Entry node should see similar entropy for different destinations"
    );
}
...
fn test_exit_node_privacy() {
    ...
    assert!(
        exit_visibility1.data_size >= message.len(),
        "Exit node should see at least original message size"
    );
    assert!(
        exit_visibility1.data_size <= message.len() + 3,
        "Exit node should see at most original message size + 3 bytes padding"
    );
    assert!(
        exit_visibility2.data_size >= message.len(),
        "Exit node should see at least original message size"
    );
    assert!(
        exit_visibility2.data_size <= message.len() + 3,
        "Exit node should see at most original message size + 3 bytes padding"
    );
    ...
    assert!(
        entropy_diff < 0.5,
        "Exit node should see similar entropy for same message"
    );
}
