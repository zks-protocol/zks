...
fn benchmark_symmetric_step(c: &mut Criterion) {
    ...
    let peer_ratchet =
        HybridRatchet::new(&shared_secret, false, HybridRatchetConfig::default()).unwrap();
    ratchet
        .set_peer_public_key(peer_ratchet.our_public_key().to_vec())
        .unwrap();
...
fn benchmark_chain_throughput(c: &mut Criterion) {
    ...
    ratchet
        .set_peer_public_key(peer.our_public_key().to_vec())
        .unwrap();
