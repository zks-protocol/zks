...
use zks_crypt::erasure_ratchet::{ErasureCodec, ErasureConfig};
...
fn benchmark_overhead(c: &mut Criterion) {
    c.bench_function("overhead-stats", |b| {
        b.iter(|| {
            ...
            println!(
                "Minimal:   k={}, n={} -> {:.0}% overhead, {:.0}% loss tolerance",
                minimal.original_count,
                minimal.total_shards(),
                minimal.overhead_factor() * 100.0,
                minimal.loss_tolerance() * 100.0
            );
            println!(
                "Balanced:  k={}, n={} -> {:.0}% overhead, {:.0}% loss tolerance",
                balanced.original_count,
                balanced.total_shards(),
                balanced.overhead_factor() * 100.0,
                balanced.loss_tolerance() * 100.0
            );
            println!(
                "Resilient: k={}, n={} -> {:.0}% overhead, {:.0}% loss tolerance",
                resilient.original_count,
                resilient.total_shards(),
