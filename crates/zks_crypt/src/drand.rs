...
use crate::constant_time::{ct_eq, ct_eq_usize, ct_gt_u64, ct_is_zero, ct_lt_u64, ct_lt_usize};
...
fn validate_drand_entropy(
    response: &DrandResponse,
    randomness_bytes: &[u8],
) -> Result<(), DrandError> {
    // Basic validation
    ...
    if ct_is_zero(randomness_bytes) {
        return Err(DrandError::ParseError(
            "All-zero entropy detected".to_string(),
        ));
    }
    ...
    if ct_eq(randomness_bytes, &all_ff) {
        return Err(DrandError::ParseError(
            "All-FF entropy detected".to_string(),
        ));
    }
    ...
    if ct_lt_usize(sig_bytes.len(), 48) {
        return Err(DrandError::ParseError(
            "Decoded signature too short for BLS".to_string(),
        ));
    }
    ...
    match verify_drand_bls_signature(response.round, &sig_bytes, None, scheme) {
        Ok(true) => {
            info!(
                "✅ BLS signature verification PASSED for drand round {} (scheme: {:?})",
                response.round, scheme
            );
        }
        Ok(false) => {
            ...
            match verify_drand_bls_signature(response.round, &sig_bytes, None, alt_scheme) {
                ...
                Err(e) => {
                    ...
                    error!(
                        "🚨 SECURITY: BLS verification error for round {}: {}",
                        response.round, e
                    );
                    return Err(DrandError::ParseError(format!(
        ...
        Err(e) => {
            ...
            return Err(DrandError::ParseError(format!(
                "BLS signature verification error: {}. Rejecting unverified entropy.",
                e
            )));
        }
    }
...
impl DrandEntropy {
    ...
    async fn fetch_fresh(&self) -> Result<[u8; 32], DrandError> {
                ...
                match self.client.get(&url).send().await {
                    Ok(response) => {
                        ...
                        match response.json::<DrandResponse>().await {
                            ...
                            Err(e) => {
                                let error_msg =
                                    format!("Failed to parse response from {}: {}", base_url, e);
                                warn!("{}", error_msg);
        ...
        Err(last_error.unwrap_or_else(|| {
            DrandError::NetworkError("All drand endpoints unavailable".to_string())
        }))
    }

    /// Validate drand response and extract randomness
    async fn validate_and_process_response(
        &self,
        drand_response: &DrandResponse,
    ) -> Result<[u8; 32], DrandError> {
    ...
    pub async fn fetch_round(&self, round: u64) -> Result<[u8; 32], DrandError> {
                ...
                match self.client.get(&url).send().await {
                    Ok(response) => {
                        ...
                        match response.json::<DrandResponse>().await {
                            ...
                            Err(e) => {
                                let error_msg =
                                    format!("Failed to parse JSON for round {}: {}", round, e);
                                warn!("{}", error_msg);
                                last_error = Some(DrandError::ParseError(error_msg));
                            }
                        }
                    ...
                    Err(e) => {
                        let error_msg =
                            format!("Network error from {} for round {}: {}", base_url, round, e);
                        warn!("{}", error_msg);
                        last_error = Some(DrandError::NetworkError(error_msg));
                    }
                }
        ...
        let error_msg = format!(
            ...
            round, self.config.max_retries, last_error
        );
        warn!("{}", error_msg);

        // Return the last error, or a generic error if somehow we got here
        Err(last_error.unwrap_or_else(|| {
            DrandError::NetworkError(format!(
                "All drand endpoints unavailable for round {}",
                round
            ))
        }))
    }
                    ...
                    Ok(response) => {
                        ...
                        match response.json::<DrandResponse>().await {
                            Ok(drand_response) => {
                                ...
                                // Decode signatures
                                let signature =
                                    hex::decode(&drand_response.signature).map_err(|e| {
                                        DrandError::ParseError(format!(
    ...
    pub async fn get_entropy_with_fallback(&self) -> Result<[u8; 32], DrandError> {
        ...
        match self.get_entropy().await {
            ...
            Err(e) => {
                ...
                info!(
                    "Using OS random fallback entropy (timestamp: {})",
                    timestamp
                );
                Ok(fallback_entropy)
            }
    ...
    pub async fn get_mixed_entropy_with_fallback(
        &self,
        user_data: &[u8],
    ) -> Result<[u8; 32], DrandError> {
        ...
        for base_url in &self.config.api_urls {
                ...
                .build()
            {
                ...
                Err(e) => {
                    results.push((
                        base_url.clone(),
                        EndpointHealth::Failed(format!("Client error: {}", e)),
                    ));
                    continue;
                }
            };
                        ...
                        match response.json::<DrandResponse>().await {
                            Ok(drand_response) => {
                                ...
                                match validate_drand_entropy(&drand_response, &randomness_bytes) {
                                    Ok(()) => {
                                        results.push((base_url.clone(), EndpointHealth::Healthy))
                                    }
                                    Err(e) => results.push((
                                        base_url.clone(),
                                        EndpointHealth::Invalid(format!(
                                            "Validation failed: {}",
                                            e
                                        )),
                                    )),
                            ...
                            Err(e) => results.push((
                                base_url.clone(),
                                EndpointHealth::Failed(format!("Parse error: {}", e)),
                            )),
                        }
                    } else {
                        results.push((
                            base_url.clone(),
                            EndpointHealth::Failed(format!("HTTP {}", response.status())),
                        ));
                    }
                }
                Err(e) => results.push((
                    base_url.clone(),
                    EndpointHealth::Failed(format!("Network error: {}", e)),
                )),
            }
        }
    ...
    pub async fn save_to_block(
        ...
        info!(
            "Fetching drand rounds {}-{} for entropy block",
            start_round, end_round
        );
        ...
        if !block.verify_integrity() {
            return Err(DrandError::ParseError(
                "Entropy block integrity verification failed".to_string(),
            ));
        }
        ...
        match block.save_to_file(output_path) {
            ...
            Err(e) => Err(DrandError::NetworkError(format!(
    ...
    pub async fn load_from_block(
        ...
        match EntropyBlock::load_from_file(block_path) {
            ...
            Err(e) => Err(DrandError::ParseError(format!(
    ...
    async fn fetch_round_with_response(
        &self,
        round: u64,
    ) -> Result<([u8; 32], DrandResponse), DrandError> {
...
mod tests {
    ...
    async fn test_mixed_entropy_uniqueness() {
        ...
        assert_ne!(
            entropy1, entropy2,
            "Different users should get different entropy"
        );
    }
}
