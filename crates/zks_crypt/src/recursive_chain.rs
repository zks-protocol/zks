...
impl RecursiveChain {
    ...
    pub fn advance(&mut self, new_entropy: &[u8]) -> [u8; 32] {
        ...
        input.extend_from_slice(if self.is_alice {
            &*self.alice_key
        } else {
            &*self.bob_key
        });
        ...
        let chain_hk = Hkdf::<Sha256>::new(Some(b"zks-chain-key"), &xor_result);
        chain_hk
            .expand(&(self.generation + 1).to_le_bytes(), self.chain.as_mut())
            .expect("HKDF expansion should not fail");
    ...
    pub fn update_peer_key(&mut self, peer_contribution: &[u8; 32]) {
        ...
        let chain_hk = Hkdf::<Sha256>::new(Some(b"zks-chain-key"), &xor_result);
        chain_hk
            .expand(&self.generation.to_le_bytes(), self.chain.as_mut())
            .expect("HKDF expansion should not fail");
