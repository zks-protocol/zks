...
pub fn encrypt_hybrid_otp(
    ...
    let cipher = ChaCha20Poly1305::new_from_slice(&*dek).map_err(|_| HybridOtpError::CipherInit)?;
...
pub fn decrypt_hybrid_otp(
    ...
    let (wrapped_dek, nonce_bytes, ciphertext) = if ct_eq(&[mode], &[0x03]) {
        ...
        let wrapped_dek: [u8; 32] = envelope[34..66]
            .try_into()
            .map_err(|_| HybridOtpError::InvalidEnvelope)?;
        let nonce_bytes: [u8; 12] = envelope[66..78]
            .try_into()
            .map_err(|_| HybridOtpError::InvalidEnvelope)?;
        ...
        let wrapped_dek: [u8; 32] = envelope[2..34]
            .try_into()
            .map_err(|_| HybridOtpError::InvalidEnvelope)?;
        let nonce_bytes: [u8; 12] = envelope[34..46]
            .try_into()
            .map_err(|_| HybridOtpError::InvalidEnvelope)?;
...
impl HybridOtp {
    ...
    pub async fn encrypt_with_sync(
        ...
        // 4. Encrypt content with ChaCha20-Poly1305(DEK)
        let cipher =
            ChaCha20Poly1305::new_from_slice(&*dek).map_err(|_| HybridOtpError::CipherInit)?;
...
mod tests {
    ...
    fn test_edge_all_zeros_plaintext() {
        ...
        assert!(
            non_zero_count > ciphertext.len() / 4,
            "Ciphertext should not be mostly zeros"
        );
    ...
    fn test_edge_truncated_envelope() {
        ...
        for truncate_len in [1, 10, 30, 45, 46] {
            ...
            assert!(
                result.is_err(),
                "Should fail with truncated envelope at -{}",
                truncate_len
            );
        }
    }
    ...
    fn test_edge_repeated_same_data() {
        ...
        for i in 0..100 {
            ...
            assert!(
                !envelopes.contains(&envelope),
                "Duplicate envelope at iteration {}",
                i
            );
