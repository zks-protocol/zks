...
impl<T> Hasher for NoHashHasher<T> {
    fn finish(&self) -> u64 {
        ...
        if self.2 {
            tracing::error!(
                "🚨 SECURITY: NoHashHasher used incorrectly - returning error sentinel"
            );
    ...
    fn write(&mut self, bytes: &[u8]) {
        ...
        debug_assert!(
            false,
            "NoHashHasher::write() called - this indicates incorrect usage with non-u64 types"
        );
        tracing::error!(
    ...
    fn write_u64(&mut self, n: u64) {
        #[cfg(debug_assertions)]
        {
            ...
            if std::mem::size_of::<T>() != std::mem::size_of::<u64>() {
                panic!(
                    "NoHashHasher<T>::write_u64() called with T != u64 - potential collision risk"
                );
            }
        }
        self.0 = n
